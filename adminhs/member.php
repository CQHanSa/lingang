...
<form name="form" id="form" method="post" action="member_save.php">
	<table width="100%" border="0" cellpadding="0" cellspacing="0" class="dataTable">
		...
		<?php
		$dopage->GetPage("SELECT * FROM `#@__member`",10);
		...
		<tr align="left" class="dataTr">
			...
			<td><span class="thumbs" style="width:48px;">
            <?php
            if($row['avatar'] != ''){
				$avatar='/'.$row['avatar'];
			}else{
				$avatar='/images/nopic.jpg';	
			}
			?>
            <img src="<?php echo $avatar; ?>" width="48" height="48" />
            </span></td>
			...
			<td class="action endCol"><span><a href="member_save.php?action=check&id=<?php echo $row['id']; ?>&checkinfo=<?php echo $row['checkinfo']; ?>" title="点击进行审核操作"><?php echo $checkinfo; ?></a></span> | <span><a href="member_update.php?id=<?php echo $row['id']; ?>">修改</a></span> | <span class="nb"><?php if($row['usertype']=='1'){?><a href="member_save.php?action=deluser&id=<?php echo $row['id']; ?>" onClick="return confirm('系统会自动删除会员及店铺信息，确定删除商家用户吗？')">删除</a><?php }else{?><a href="member_save.php?action=del2&id=<?php echo $row['id']; ?>" onclick="return ConfDel(0)">删除</a><?php }?></span></td>
		</tr>
		<?php
		}
		?>
