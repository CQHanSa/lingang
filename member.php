...
if($a == 'login')
{
	...
	if($cfg_oauth == 'Y' && isset($method) && $method == 'callback')
	{
		...
		if(check_app_login('qq') or check_app_login('weibo'))
		{
			...
			if(empty($row['id']))
			{
				...
				setcookie('userid', AuthCode($id ,'ENCODE'), $cookie_time);
	...
	else
	{
		...
		$row = $dosql->GetOne("SELECT `id`,username,`password`,`logintime`,`loginip`,`expval`,usertype FROM `#@__member` WHERE `username`='$username' or `mobile`='$username' or `email`='$username'");
			...
			else
			{
				...
				*/
				...
				if($row['usertype'] == 0){ 
					//个人用户
					echo '<script>location.href="/member/person/";</script>';
					exit();
				}else if($row['usertype'] == 1){ 
					//商家用户
					echo '<script>location.href="/member/shop/";</script>';
					exit();
				}else if($row['usertype'] == 2){ 
					//社区用户
					echo '<script>location.href="/member/community/";</script>';
					exit();
				}
			}
...
else if($a == 'reg')
{
	...
	//验证输入数据
	if($username   == '' or $password   == '' or  $repassword == '' or $validate   == '')
	{
	...
	//验证数据准确性
	if($validate != $_SESSION['mobilecode'])
	{
		ShowMsg('验证码不正确！','-1');
		exit();
	}
	...
	*/
	...
	/*
	if(preg_match("/[^0-9a-zA-Z_@!\.-]/",$username) or preg_match("/[^0-9a-zA-Z_-]/",$password) or !preg_match("/^[\w-]+(\.[\w-]+)*@[\w-]+(\.[\w-]+)+$/", $email))
	{
		header('location:?c=reg');
		exit();
	}
